...
class Sesion extends SesionBase {
    ...
    const APLICATIVO = 'aplicativo_urano';
     ...
     */
    /*
	 * Se modifica por juusechec para consultar los usuarios sobre la base de datos dbms
	 */
    function nivelSesion() {
    	$this->sesionUsuarioId = trim($this->getValorSesion('idUsuario'));
		
    	$this->configurador->fabricaConexiones->getRecursoDB ( "appserv" );		
		$cadenaSql = $cadenaSql = $this->miSql->getCadenaSql("verificarNivelUsuarioDBMS", $this->sesionUsuarioId);

        $respuesta = $this->miConexion->ejecutarAcceso($cadenaSql);
		if($respuesta){
			return 1;//Se ajusta ese nivel de sesión para evitar consultar el usuario;
		} else {
			return 0;
		}
        // $this->sesionUsuarioId = trim($this->getValorSesion('idUsuario'));
